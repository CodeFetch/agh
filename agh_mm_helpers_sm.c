...
static void agh_mm_sm_report_error(struct agh_state *mstate, gchar *message) {
	...
	event = agh_cmd_event_alloc(NULL);
	agh_cmd_answer_set_status(event, AGH_CMD_ANSWER_STATUS_FAIL);
	agh_cmd_answer_addtext(event, AGH_MM_SM_GENERIC_FAILURE, TRUE);

	if (message)
		agh_cmd_answer_addtext(event, message, TRUE);
	else if (mmstate->gerror) {
		if (mmstate->gerror->message)
			agh_cmd_answer_addtext(event, mmstate->gerror->message, TRUE);
		else
...
static void agh_mm_sm_sim_unlock_send_pin_res(MMSim *sim, GAsyncResult *res, struct agh_state *mstate) {
	...
	if (!sres) {
		event = agh_cmd_event_alloc(NULL);
		agh_cmd_answer_set_status(event, AGH_CMD_ANSWER_STATUS_FAIL);
		agh_cmd_answer_addtext(event, AGH_MM_SIM_EVENT_NAME, TRUE);
		agh_cmd_answer_addtext(event, mm_sim_get_identifier(sim), TRUE);
		agh_cmd_answer_addtext(event, "send_pin_failure", TRUE);

		if (mstate->mmstate->gerror) {
			if (mstate->mmstate->gerror->message)
				agh_cmd_answer_addtext(event, mstate->mmstate->gerror->message, TRUE);
			else
				agh_cmd_answer_addtext(event, "unknown", TRUE);
...
void agh_mm_sm_report_failure_modem(struct agh_state *mstate, MMModem *modem, gchar *mmarker) {
	...
	modem_idx = NULL;
	if (mmarker)
		agh_cmd_answer_addtext(event, mmarker, TRUE);
	else
		agh_cmd_answer_addtext(event, AGH_MM_SM_MODEM_INIITSTATE_FAILURE, TRUE);
...
static void agh_mm_sm_report(struct agh_state *mstate, guint status, gchar *eventname, gchar *mmarker, gchar *name, gchar *reason, gboolean is_data) {
	...
	agh_cmd_answer_addtext(event, eventname, TRUE);
	...
	agh_cmd_answer_addtext(event, name, TRUE);
